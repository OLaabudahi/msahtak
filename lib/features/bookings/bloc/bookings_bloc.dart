...
class BookingsBloc extends Bloc<BookingsEvent, BookingsState> {
  ...
  Future<void> _onStarted(BookingsStarted event, Emitter<BookingsState> emit) async {
    emit(state.copyWith(loading: true, error: null));
    try {
      final data = await repo.fetchBookings();
      emit(state.copyWith(loading: false, all: data, error: null));
    } catch (e) {
      emit(state.copyWith(loading: false, error: e.toString(), all: const []));
    }
  }

  /// ✅ دالة: تغيير Segmented (Upcoming / Past)
  void _onSegmentChanged(BookingsSegmentChanged event, Emitter<BookingsState> emit) {
    emit(state.copyWith(segmentIndex: event.index));
  }

  /// ✅ دالة: ريفريش (Pull to refresh)
  Future<void> _onRefresh(BookingsRefreshRequested event, Emitter<BookingsState> emit) async {
    try {
      final data = await repo.fetchBookings();
      emit(state.copyWith(all: data, error: null));
    } catch (e) {
