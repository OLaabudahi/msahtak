...
import '../services/space_favorites_firebase_service.dart';
import 'space_details_source.dart';

class SpaceDetailsFirebaseSource implements SpaceDetailsSource {
  final FirebaseFirestore firestore;
  final SpaceFavoritesFirebaseService favoritesService;

  SpaceDetailsFirebaseSource({
    FirebaseFirestore? firestore,
    SpaceFavoritesFirebaseService? favoritesService,
  })  : firestore = firestore ?? FirebaseFirestore.instance,
        favoritesService = favoritesService ?? SpaceFavoritesFirebaseService();
  ...
  Future<SpaceDetails> fetchSpaceDetails(String spaceId) async {
    ...
    final reviewSummary = ReviewSummary(
      ...
      topPositives:
          (rsMap['top_positives'] as List?)?.cast<String>() ?? const [],
      repeatedNegatives:
    ...
    final policiesMap =
        (policiesObj is Map) ? policiesObj.cast<String, dynamic>() : <String, dynamic>{};

    final rawSections =
        (d['policySections'] as List?) ?? (policiesMap['sections'] as List?) ?? const [];
      ...
      sections: rawSections
          ...
          .map((s) {
            ...
            return PolicySection(
              ...
              bullets: (m['bullets'] as List?)?.map((b) => b.toString()).toList() ?? const [],
            );
          })
          .toList(),
    );
  ...
  Future<void> addFavorite({
    ...
    return favoritesService.addFavorite(
      ...
      payload: {
        ...
        'lat': details.lat,
        'lng': details.lng,
        'image': details.imageAssets.isNotEmpty ? details.imageAssets.first : null,
      },
    );
  }
  ...
  Future<void> removeFavorite(String spaceId) {
    return favoritesService.removeFavorite(spaceId);
  }

  @override
  Future<List<String>> getFavorites() async {
    final items = await favoritesService.getFavorites();
    return items
        .map((e) => (e['spaceId'] ?? '').toString())
